...
### 应用图标
柔粉蓝渐变的"a"字 + 周围浮散的圆角方块，从 `aLaunchpad/Resources/AppIcon.png` 通过 `set_icon.sh` 打包成 10 个 macOS 标准尺寸。如果 `AppIcon.png` 不存在，`build.sh` 会自动回退到 `Scripts/MakeIcon.swift` 程序化生成的占位图标（拉丝银 + 3×3 格子），保证项目在任何情况下都能构建。
...
```bash
# 重新打包仓库自带的 AppIcon.png（也可换成你自己的 PNG）
...
### 为什么同时保留 PNG 图标和程序化回退？

发布用的图标位于 `aLaunchpad/Resources/AppIcon.png`，构建时打包进 bundle —— 它是仓库里唯一的二进制资源。`Scripts/MakeIcon.swift` 仍然保留，是为了保证项目永远能构建：如果 PNG 被删或替换后忘记 commit，`build.sh` 会自动回退到程序化占位图标（拉丝银 + 3×3 格子），bundle 永远不会缺图标。
